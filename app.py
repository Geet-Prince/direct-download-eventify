...
global _cached_fests_data_all, _cache_fests_timestamp_all; cached_fests_data_all = None; cache_fests_timestamp_all = None; print("INFO: All fests cache invalidated.")
safe_base="".join(c if c.isalnum() or c in [' ','','-'] else "" for c in str(fest_name)).strip() or "fest_event";
safe_sheet_title=f"{safe_base[:80]}{fest_id}"; event_headers=['UniqueID','Name','Email','Mobile','College','Present','Timestamp'];
...
if 'club_id' not in session: flash("Login required.", "warning"); return redirect(url_for('club_login'))
try:
g_client, , , _ = get_sheet_objects_cached()
all_fests_data = get_all_fests_cached()
fest_info = next((f for f in all_fests_data if str(f.get('FestID','')) == fest_id), None)
...
safe_name = "".join(c if c.isalnum() or c in [' ','','-'] else "" for c in str(fest_info.get('FestName','Event'))).strip() or "fest_event"
sheet_title = f"{safe_name[:80]}{fest_info.get('FestID','')}"
...
try:
g_client, , , _ = get_sheet_objects_cached(); all_fests_data = get_all_fests_cached()
fest_info = next((f for f in all_fests_data if str(f.get('FestID','')) == fest_id), None)
if not fest_info: flash("Event not found.", "danger"); return redirect(url_for('club_dashboard'))
if str(fest_info.get('ClubID','')) != session['club_id']: flash("Unauthorized export.", "danger"); return redirect(url_for('club_dashboard'))
safe_name = "".join(c if c.isalnum() or c in [' ','','-'] else "" for c in str(fest_info.get('FestName','Event'))).strip() or "fest_event"
spreadsheet_title = f"{safe_name[:80]}{fest_info.get('FestID','')}"
try:
...
safe_base="".join(c if c.isalnum() or c in [' ','','-'] else "" for c in str(fest_info.get('FestName','Event'))).strip() or "fest_event";
individual_sheet_title=f"{safe_base[:80]}{fest_info['FestID']}"; event_headers=['UniqueID','Name','Email','Mobile','College','Present','Timestamp'];
...
safe_user_name_for_file = "".join(c if c.isalnum() or c in ['','-',' '] else "" for c in str(name)).strip().replace(' ', '') or "user"
safe_fest_name_for_file = "".join(c if c.isalnum() or c in ['','-',' '] else "" for c in str(fest_info.get('FestName','Event'))).strip().replace(' ', '') or "event"
download_filename = f"{safe_user_name_for_file}QR_for{safe_fest_name_for_file}_{user_id[:4]}.png"
...
safe_base="".join(c if c.isalnum() or c in [' ','','-'] else "" for c in str(valid_event.get('FestName','Event'))).strip() or "fest_event";
session['security_event_sheet_title']=f"{safe_base[:80]}{valid_event.get('FestID','')}";
...
app.run(debug=True, host='0.0.0.0', port=port, use_reloader=True) # Set use_reloader=False for production testing on Rendergive me the full code implement inddian time and security scan start at any time but ends at the time of event ends
